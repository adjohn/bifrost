...
const MCPFilterSchema = `{
  ...
  "properties": {
    ...
    "status": {"type": "array", "items": {"type": "string", "minLength": 1}, "minItems": 1, "maxItems": 50, "description": "success, error, or processing."},
  ...
  "required": ["filters"]
		...
		execute: func(ctx context.Context, deps *Deps, args map[string]any) (any, error) {
			...
			return map[string]any{
				...
				"has_more": result.HasLogs && len(rows) >= limit,
...
func parseMCPFilters(raw map[string]any) (*logstore.MCPToolLogSearchFilters, error) {
	...
	if raw["content_search"] != nil {
		text, ok := raw["content_search"].(string)
		if !ok {
			return nil, fmt.Errorf("content_search must be a string, got %T", raw["content_search"])
		}
		filters.ContentSearch = text
	}
	return filters, nil
}
