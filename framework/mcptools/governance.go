  ...
  "required": ["name"]
		...
		execute: func(ctx context.Context, deps *Deps, args map[string]any) (any, error) {
			...
			if reloader := requireReloader(deps); reloader != nil {
				if reloaded, err := reloader.ReloadVirtualKey(ctx, vk.ID); err == nil && reloaded != nil {
					vk = reloaded
				}
			}
...
func rotateVirtualKeyTool() Tool {
  ...
  "required": ["virtual_key_id"]
		...
		execute: func(ctx context.Context, deps *Deps, args map[string]any) (any, error) {
			...
			if cfg, err := gov.GetClientConfig(ctx); err == nil && cfg != nil {
...
func createBudgetTool() Tool {
  ...
  "required": ["max_limit", "reset_duration"]
		...
		execute: func(ctx context.Context, deps *Deps, args map[string]any) (any, error) {
			...
			if reloader := requireReloader(deps); reloader != nil {
				if hasTeam {
					_, _ = reloader.ReloadTeam(ctx, teamID)
				} else {
					_, _ = reloader.ReloadCustomer(ctx, customerID)
				}
			}
			return describeBudget(budget), nil
		},
		...
		execute: func(ctx context.Context, deps *Deps, args map[string]any) (any, error) {
			...
			if reloader := requireReloader(deps); reloader != nil {
				if budget.TeamID != nil {
					_, _ = reloader.ReloadTeam(ctx, *budget.TeamID)
				}
				if budget.CustomerID != nil {
					_, _ = reloader.ReloadCustomer(ctx, *budget.CustomerID)
				}
				if budget.VirtualKeyID != nil {
					_, _ = reloader.ReloadVirtualKey(ctx, *budget.VirtualKeyID)
				}
			}
			return describeBudget(budget), nil
		},
