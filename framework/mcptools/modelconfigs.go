    ...
    "limit": {"type": "integer", "minimum": 1, "maximum": 20}
		...
		execute: func(ctx context.Context, deps *Deps, args map[string]any) (any, error) {
			...
			rows, total, err := gov.GetModelConfigsPaginated(ctx, configstore.ModelConfigsQueryParams{
				Limit: limit, Search: search, Scope: scope, ScopeID: scopeID, Provider: provider,
			})
			if err != nil {
...
func createModelConfigTool() Tool {
  ...
  "required": ["model_name"]
		...
		execute: func(ctx context.Context, deps *Deps, args map[string]any) (any, error) {
			...
			if rateLimit != nil {
				if err := gov.CreateRateLimit(ctx, rateLimit); err != nil {
					return nil, fmt.Errorf("create rate limit failed: %w", err)
				}
				mc.RateLimitID = &rateLimit.ID
				mc.RateLimit = rateLimit
			}
			if err := gov.CreateModelConfig(ctx, mc); err != nil {
				if errors.Is(err, configstore.ErrAlreadyExists) {
					return nil, fmt.Errorf("a model config for %q already exists in this scope", modelName)
				}
				return nil, fmt.Errorf("create model config failed: %w", err)
			}
		...
		execute: func(ctx context.Context, deps *Deps, args map[string]any) (any, error) {
			...
			if patch != nil {
				if mc.RateLimitID != nil && *mc.RateLimitID != "" {
					existing, err := gov.GetRateLimit(ctx, *mc.RateLimitID)
					if err != nil {
					...
					if err := gov.UpdateRateLimit(ctx, existing); err != nil {
						return nil, fmt.Errorf("update rate limit failed: %w", err)
					}
					mc.RateLimit = existing
				} else {
					if err := gov.CreateRateLimit(ctx, patch); err != nil {
						return nil, fmt.Errorf("create rate limit failed: %w", err)
					}
			...
			if err := gov.UpdateModelConfig(ctx, mc); err != nil {
				return nil, fmt.Errorf("update model config failed: %w", err)
			}
			if reloader := requireReloader(deps); reloader != nil {
				if reloaded, err := reloader.ReloadModelConfig(ctx, mc.ID); err == nil && reloaded != nil {
					mc = reloaded
