...
func updateFeatureFlagTool() Tool {
  ...
  "required": ["id", "enabled"]
		...
		execute: func(ctx context.Context, deps *Deps, args map[string]any) (any, error) {
			...
			if err := gov.UpsertFeatureFlag(ctx, id, enabled, time.Now().UnixNano()); err != nil {
				return nil, fmt.Errorf("update feature flag failed: %w", err)
			}
			return map[string]any{
				"id":      id,
				"enabled": enabled,
				"source":  "db",
				"note":    "persisted; in-memory flag store is unavailable so this process may not see it until restart",
			}, nil
		},
	}
}
