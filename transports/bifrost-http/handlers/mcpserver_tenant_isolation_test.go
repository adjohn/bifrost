...
func TestBifrostMCP_TenantIsolation(t *testing.T) {
	...
	t.Run("customer wins over team when both are stamped", func(t *testing.T) {
		// A key under team A but attributed to customer Y: the customer is the more specific
		// statement of who pays, and it is what the scope follows.
		assertScope(t, identityAdmitter{teamID: "team-a", customerID: "cust-y"}, owners["cust-y"])
	})
